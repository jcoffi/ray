...
#   sudo sed -i "s/_tailscale0_/_eth0_/g" /crate/config/crate.yml
#   ray start --address='nexus:6379' --resources='{"'"$LOCATION"'": '$(nproc)'}' --disable-usage-stats --dashboard-host 0.0.0.0 --node-ip-address $IPADDRESS --node-name $HOSTNAME #--object-store-memory=$ray_object_store
  ...
  ray start --address='nexus:6379' --num-cpus=1 --disable-usage-stats --dashboard-host 0.0.0.0 --node-ip-address $HOSTNAME --node-name $HOSTNAME &
